...
// RuleType is the discriminator for recurrence strategies.
...
// DayParity distinguishes even/odd days of the month.
...
// Rule stores the recurrence configuration attached to a parent task.
type Rule struct {
	ID            int64
	TaskID        int64
	RuleType      RuleType
	IntervalDays  *int        // daily only
	MonthDay      *int        // monthly only
	SpecificDates []time.Time // specific_dates only
	DayParity     *DayParity  // even_odd only
	StartDate     time.Time
...
// Occurrences returns all dates in [from, to] (inclusive) that match the rule.
